...
import com.advance.auth.exception.EntityNotFoundException;
import com.advance.auth.repository.CredentialsRepository;
...
class AuthServiceTest {
    ...
    void refresh_ShouldReturnNewTokens_WhenTokenValid() {
        ...
        Assertions.assertThat(response.getAccessToken()).isEqualTo("new_access");
    }
    ...
    void validateToken_ShouldReturnTrue_WhenTokenValid() {
        ...
        Assertions.assertThat(authService.validateToken("valid_token")).isTrue();
    }

    @Test
    void validateToken_ShouldReturnFalse_WhenTokenInvalid() {
        when(jwtService.isTokenValid("bad_token")).thenReturn(false);
        Assertions.assertThat(authService.validateToken("bad_token")).isFalse();
    }
}